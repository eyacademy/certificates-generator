...
ONLINE_LAYOUT = {
    "normal": {
        ...
        "course": {"x": 400, "y": 590, "align": "left", "size": 12, "max_width": 320},
    ...
    "small": {
        ...
        "course": {"x": 400, "y": 590, "align": "left", "size": 11, "max_width": 320},
...
LAYOUT_BASE = {
    "print": {"w": 842.0, "h": 595.0},
    "online": {"w": 595.0, "h": 842.0},
}
...
KEY_ALIASES = {
    ...
    "course": {"название тренинга", "название", "course", "course name", "course_name", "название тренинга/название", "название/название тренинга"},
...
def normalize_online(v: str) -> bool:
    v = (v or "").strip().lower()
    if v in {"да", "yes", "true", "1", "y", "онлайн", "online"}: return True
    if v in {"нет", "no", "false", "0", "n", "оффлайн", "offline", "off"}: return False
    return False
...
def parse_dates(s: str) -> Dict[str, Optional[int]]:
    ...
    months_tokens = [("January",1),("February",2),("March",3),("April",4),("May",5),("June",6),("July",7),("August",8),("September",9),("October",10),("November",11),("December",12)]
    def detect_month(token: str) -> Optional[int]:
        if token.isdigit() and 1 <= int(token) <= 12: return int(token)
        for pref, m in months_tokens:
            if token.startswith(pref): return m
        return None
    ...
    if rng and month_idx:
        return {"d1": int(rng.group(1)), "m1": month_idx[0][1], "d2": int(rng.group(2)), "m2": month_idx[0][1], "y": y}
...
def format_dates_for_jinja(p: Dict[str, Optional[int]]) -> Dict[str, str]:
    ...
    result = {"Имя":"", "Фамилия":"", "Тренинг":"", "Год": str(y), "Город":"Москва"}
    if d1 and d2:
        result.update({"Дата1": str(d1), "Дата2": str(d2), "Месяц1": MONTH_GEN[m1], "Месяц2": MONTH_GEN[m2]})
    else:
...
def fit_font_size_to_width(text: str, base_size: int, max_width: Optional[int]) -> int:
    if not max_width: return base_size
    size = base_size
...
def format_dates_line(p: Dict[str, Optional[int]], city: Optional[str] = None) -> str:
    d1, m1, d2, m2, y = p["d1"], p["m1"], p["d2"], p["m2"], p["y"]
    core = f"{d1} и {d2} {MONTH_GEN[m1]} {y}" if d1 and d2 and m1 == m2 else (f"{d1} {MONTH_GEN[m1]} — {d2} {MONTH_GEN[m2]} {y}" if d1 and d2 else f"{d1} {MONTH_GEN[m1]} {y}")
    city = (city or "").strip()
...
def wrap_text_to_width(text: str, size: int, max_width: int) -> List[str]:
    ...
    for w in words:
        ...
        if string_width_pt(cand, size) <= max_width or not line:
            line = cand
        else:
            lines.append(line)
            line = w
...
def build_overlay_pdf_bytes(first_name: str, last_name: str, course: str, parsed_dates: Dict[str, Optional[int]], cert_id: str, variant: str, page_w: float, page_h: float, group: str, city: Optional[str] = None, offsets: Optional[Dict[str, float]] = None, draw: Tuple[str, ...] = ("name","course","dates","id"), debug: bool = False) -> bytes:
    layout = (ONLINE_LAYOUT if group == "online" else LAYOUT)[variant]
    base = LAYOUT_BASE.get(group, LAYOUT_BASE["print"])
    ...
    if "name" in draw:
        ...
        draw_aligned_text(cnv, name_cfg["x"] * sx + name_dx, name_cfg["y"] * sy + name_dy, full_name, name_cfg.get("align","center"), name_cfg.get("size",24), font_name=name_cfg.get("font"), page_w=page_w, page_h=page_h)
    ...
    if "course" in draw:
        ...
        lines = wrap_text_to_width(course, fitted_size, max_width)
        line_height = int(round(fitted_size * 1.25))
        x0 = course_cfg["x"] * sx + course_dx
        y0 = course_cfg["y"] * sy + course_dy
        for i, line in enumerate(lines):
            draw_aligned_text(cnv, x0, y0 - i * line_height, line, course_cfg.get("align","left"), fitted_size, page_w=page_w, page_h=page_h)
    ...
    if "dates" in draw:
        ...
        draw_aligned_text(cnv, dates_cfg["x"] * sx + dates_dx, dates_cfg["y"] * sy + dates_dy, dates_line, dates_cfg.get("align","center"), dates_cfg.get("size",14), page_w=page_w, page_h=page_h)

    if "id" in draw:
        id_cfg = layout["id"]
        id_dx = (offsets or {}).get("id_dx", 0.0)
        id_dy = (offsets or {}).get("id_dy", 0.0)
        draw_aligned_text(cnv, id_cfg["x"] * sx + id_dx, id_cfg["y"] * sy + id_dy, cert_id, id_cfg.get("align","right"), id_cfg.get("size",10), page_w=page_w, page_h=page_h)

    if debug:
        cnv.setStrokeColor(colors.red)
        cnv.setFillColor(colors.red)
        def cross(x, y, label):
            cnv.line(x-5, y, x+5, y); cnv.line(x, y-5, x, y+5)
            cnv.setFont(FONT_NAME, 8); cnv.drawString(x+6, y+6, label)
        for key in ("name","course","dates","id"):
            if key in draw:
                cfg = layout[key]
                cross(cfg["x"] * sx, cfg["y"] * sy, key)
...
def docx_to_pdf_cached(docx_path: str) -> str:
    ...
    libreoffice_paths = ["soffice", r"C:\Program Files\LibreOffice\program\soffice.exe", r"C:\Program Files (x86)\LibreOffice\program\soffice.exe", r"C:\LibreOffice\program\soffice.exe"]
    cmd = None
    for path in libreoffice_paths:
        try:
            ...
            continue
    if cmd is None:
        raise RuntimeError("LibreOffice not found. Please install or add to PATH.")
    profile_dir = os.path.join(out_dir, "lo_profile")
    os.makedirs(profile_dir, exist_ok=True)
    profile_url = "file:///" + os.path.abspath(profile_dir).replace("\\", "/").lstrip("/")
    cmd_with_profile = [cmd[0], "--headless", "--norestore", "--nolockcheck", f"-env:UserInstallation={profile_url}", "--convert-to", "pdf", "--outdir", out_dir, abs_docx]
...
def render_docx_template(docx_path: str, context: Dict[str, str], adjust_online_course_indent: bool = False, course_indent_pts: int = 18) -> bytes:
    ...
    if adjust_online_course_indent:
        try:
            ...
            for p in d.paragraphs:
                txt = (p.text or "")
                if not name_para and target_name and target_name in txt: name_para = p
                if not course_para and target_course and target_course in txt: course_para = p
                if name_para and course_para: break
            name_left = name_para.paragraph_format.left_indent if name_para else None
            name_right = name_para.paragraph_format.right_indent if name_para else None
            if course_para is not None:
                pf = course_para.paragraph_format
                pf.alignment = WD_ALIGN_PARAGRAPH.RIGHT
                if name_right is not None: pf.right_indent = name_right
                if name_left is not None: pf.left_indent = name_left
                try: pf.first_line_indent = Pt(0)
                except Exception: pass
                COURSE_EXTRA_SHIFT_PTS = 12
                if pf.right_indent is None:
                    pf.right_indent = Pt(COURSE_EXTRA_SHIFT_PTS)
                else:
                    try: pf.right_indent = Pt(pf.right_indent.pt + COURSE_EXTRA_SHIFT_PTS)
                    except Exception: pf.right_indent = Pt(COURSE_EXTRA_SHIFT_PTS)
                d.save(temp_docx.name)
        except Exception as e:
            logger.warning(f"Course right-align adjust skipped: {e}")
...
def build_blank_pdf_from_docx_template(docx_path: str) -> str:
    ...
    if cache_key in DOCX_TO_PDF_CACHE: return DOCX_TO_PDF_CACHE[cache_key]
    doc = DocxTemplate(docx_path)
    try:
        doc.render({})
    except Exception:
        doc.render({"Имя":"","Фамилия":"","Тренинг":"","Дата1":"","Дата2":"","Месяц1":"","Месяц2":"","Год":"","Город":"","Идентификатор":""})
    temp_docx = tempfile.NamedTemporaryFile(suffix=".docx", delete=False)
    doc.save(temp_docx.name)
    temp_docx.close()
    pdf_path = docx_to_pdf_cached(temp_docx.name)
...
def get_template_pdf_path(docx_name: str) -> str:
    ...
    if os.path.exists(prebuilt_pdf): return prebuilt_pdf
...
def merge_overlay_bytes(base_pdf_bytes: bytes, overlay_pdf_bytes: bytes) -> bytes:
...
def snapshot(state: ProgressState) -> Dict[str, object]:
    ...
    return {"total": state.total, "processed": state.processed, "percent": percent, "stage": state.stage, "message": state.message, "errors": state.errors}
...
def sample_excel():
    ...
    return FileResponse(sample_path, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", filename="Template_Certificates.xlsx")

@app.get("/check-templates")
def check_templates():
    missing_templates, available_templates = [], []
    ...
    return {"available_templates": available_templates, "missing_templates": missing_templates, "templates_dir": TEMPLATES_DIR, "templates_dir_exists": os.path.exists(TEMPLATES_DIR)}
...
@app.post("/generate")
async def generate(csv_file: UploadFile = File(...), mode: str = Form(...), job_id: Optional[str] = Form(None)):
    try:
            ...
            with zipfile.ZipFile(mem_zip, "w", zipfile.ZIP_DEFLATED) as zf:
                for row_num, row in enumerate(rows_list, 1):
                    try:
                        ...
                        async def render_one(docx_path=docx_path, context=context, cert_id=cert_id, last_name=last_name, first_name=first_name, group=group, course=course, parsed=parsed, variant=variant, city=city):
                            adjust = False
                            pdf_bytes = await loop.run_in_executor(executor, render_docx_template, docx_path, context, adjust)
                            if group == "online":
                                base_reader = PdfReader(io.BytesIO(pdf_bytes))
                                page = base_reader.pages[0]
                                page_w = float(page.mediabox.width)
                                page_h = float(page.mediabox.height)
                                overlay_bytes = build_overlay_pdf_bytes(first_name, last_name, course, parsed, cert_id, variant, page_w, page_h, group, city, draw=("course",))
                                pdf_bytes = merge_overlay_bytes(pdf_bytes, overlay_bytes)
        ...
        if processed_count == 0:
            try:
                ...
                incoming_fields = []
            required = ["Имя/Name","Фамилия/Surname","Название тренинга/Название","Даты/Дата","ID/Id","(опц.) Город/City","(опц.) Страна/Country"]
            hint = "CSV распознан, но ни одной корректной строки не найдено. Проверьте заголовки и обязательные поля. Требуемые колонки: " + ", ".join(required)
        ...
        return Response(content=zip_bytes, media_type="application/zip", headers={"Content-Disposition": "attachment; filename=certificates.zip"})
    except Exception as e:
...
async def generate_async(csv_file: UploadFile = File(...), mode: str = Form(...), job_id: Optional[str] = Form(None)):
    try:
...
def download_result(job_id: str):
    ...
    try:
        ...
        pass
    return Response(content=data, media_type="application/zip", headers={"Content-Disposition": "attachment; filename=certificates.zip"})
